...
$.widget("afxflowchart.afxflowchart", {
    options: {
        canUserEditLinks: true,
        canUserMoveOperators: true,
        data: {},
        distanceFromArrow: 3,
        defaultOperatorClass: 'operator-card',
    ...
    globalId: null,
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('pointerdown mousedown touchstart', '.operator-card', function (e) {
                e.stopImmediatePropagation();
            });

            this.objs.layers.operators.on('click', '.operator-card', function (e) {
                if ($(e.target).closest('.flowchart-operator-connector').length == 0) {
                    self.selectOperator($(this).data('operator_id'));
                }
            });

            this.objs.layers.operators.on('click', '.dot', function () {
                var $this = $(this);
                if (self.options.canUserEditLinks) {
                    console.log($this.closest(".dots-holder").data('connector_type'))
                    self._connectorClicked($this.closest('.operator-card').data('operator_id'), $this.data('connector'), $this.data('sub_connector'), $this.closest(".dots-holder").data('connector_type'));
                }
            });

            this.objs.layers.operators.on('click', '.remove-operator', function (e) {
                e.stopImmediatePropagation()
                var $this = $(this);
                var currentOperatorId = $this.closest('.operator-card').data('operator_id')
                var confirm_deletion= confirm("Please Confirm the deletion");
                if (confirm_deletion == true) {
                    self._deleteOperator(currentOperatorId);
                }
            });
            ...
            this.objs.layers.operators.on('mouseover', '.operator-card', function (e) {
                self._operatorMouseOver($(this).data('operator_id'));
            });

            this.objs.layers.operators.on('mouseout', '.operator-card', function (e) {
        ...
        _refreshLinkPositions: function (linkId) {
            ...
            linkData.internal.els.path.setAttribute("d", 'M' + bezierFromX + ',' + (fromY) + ' C' + (fromX + offsetFromX + distanceFromArrow + bezierIntensity - 100) + ',' + fromY + ' ' + (toX - bezierIntensity+100) + ',' + toY + ' ' + bezierToX + ',' + toY);
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator = $('<div class="flowchart-operator text-center"></div>');
            $operator.addClass(infos.class);
            $operator_remover = $('<div class="remove-operator pull-right">remove</div>');
            var $operator_title = $('<div class="flowchart-operator-title"></div>');
            var $operator_title_section = $('<div class="card-block flowchart-operator-title"><h4 class="card-title">'+infos.title+'</h4><p class="card-text">With supporting text below as a natural lead-in to additional content.</p></div>')
            // $operator_title.html(infos.title);
            $operator_remover.appendTo($operator);
            $operator_title_section.appendTo($operator);

            var $operator_inputs_ = $('<div class="card-header"></div>');
            var $operator_outputs_ = $('<div class="card-footer"></div>');

            $operator_inputs_.prependTo($operator);
            $operator_outputs_.appendTo($operator);

            // $operator_inputs.appendTo($operator_inputs_);

            // var $operator_outputs = $('<div class="flowchart-operator-output"></div>');
            // $operator_outputs.appendTo($operator_outputs_);
            ...
            function addConnector(connectorKey, connectorInfos, $operator_container, connectorType) {
                var $operator_connector_set = $('<div class="dots-holder '+connectorType+'-holder" data-connector_type="'+connectorType+'"></div>');
            ...
            for (var key_i in infos.inputs) {
                if (infos.inputs.hasOwnProperty(key_i)) {
                    addConnector(key_i, infos.inputs[key_i], $operator_inputs_, 'inputs');
                }
            }

            for (var key_o in infos.outputs) {
                if (infos.outputs.hasOwnProperty(key_o)) {
                    addConnector(key_o, infos.outputs[key_o], $operator_outputs_, 'outputs');
                }
            }
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            var $operator_connector = $('<div class="dot flowchart-operator-connector"></div>');
            ...
            fullElement.connectorArrows[connectorKey].push($operator_connector);
            fullElement.connectorSmallArrows[connectorKey].push($operator_connector);
        },
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    drag: function (e, ui) {
                        ...
                        if (self.options.grid && ui.offset.top >24) {
                        $(".flowchart-link").removeClass("hide")

                            console.log($(ui.helper.context).offset())
                            ...
                            var $this = $(this);
                            if ( elementOffset.left - self.element.width() <= 0) {
        ...
        _click: function (x, y, e) {
            ...
            if ($target.closest('.operator-card').length == 0) {
            ...
            if ($target.closest('.flowchart-link').length) {
                var remove_link = confirm("do you want to remove this link")
                if(remove_link){
                    this.deleteLink($target.closest('.flowchart-link').data('link_id'))
                }
            }
        },

        _removeSelectedClassOperators: function () {
            this.objs.layers.operators.find('.operator-card').removeClass('selected');
        },
        ...
        removeClassOperators: function (className) {
            this.objs.layers.operators.find('.operator-card').removeClass(className);
        },
        ...
        _removeHoverClassOperators: function () {
            this.objs.layers.operators.find('.operator-card').removeClass('hover');
        },
